...
//****** remember that "Filters" create new arrays***********
//  */

let veryOldInventors = inventors.filter((inventor) => {
    return inventor.year >= 1500 && inventor <= 1599;
});
...
let inventorNames2= inventors.map ((inventor) => {
    const fullName = {};
...
// Check your work:
console.log('Exercise 2 my result: ', inventorNames2);
...
let firstLast = people.map((p)=> {
   const namesArr = p.split(", ");
    return namesArr [1] + " " + namesArr;
    // return p.split(" ,").reverse().join(" ") <--- second option for return- quicker to write. 
});
